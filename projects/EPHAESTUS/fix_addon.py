"""Fix addon.py syntax by removing bad patch and reapplying correctly"""

# Read the original addon (before patch)
with open(r'D:\EPHAESTUS\addon.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find where the bad patch starts (line 2115, index 2114)
# Remove lines from 2113 (blank line before patch) to end of patch

# Find the line with "else:" that should come after our patches
else_line_idx = None
for i in range(2112, len(lines)):
    if lines[i].strip().startswith('else:') and 'Unknown command type' in lines[i+1]:
        else_line_idx = i
        break

if else_line_idx:
    # Remove bad patch (from line 2113 to before else)
    cleaned_lines = lines[:2112] + lines[else_line_idx:]

    # Now insert correct patch with proper indentation
    patch = '''
...
'''
    ...
    # Insert patch before the else line
    final_lines = cleaned_lines[:2112] + [patch] + cleaned_lines[2112:]

    # Write fixed file
    with open(r'D:\EPHAESTUS\addon.py', 'w', encoding='utf-8') as f:
        f.writelines(final_lines)

    print("SUCCESS: Fixed addon.py with correct indentation")
    print(f"Removed bad patch from line 2113-{else_line_idx}")
    print(f"Added correct patch at line 2112")
else:
    print("ERROR: Could not find else block")
